...
# --- ГЛАВНОЕ ИЗМЕНЕНИЕ №1: ИЩЕМ ПРАВИЛЬНОЕ ИМЯ ПЕРЕМЕННОЙ ---
...
# --- ГЛАВНОЕ ИЗМЕНЕНИЕ №2: ПРАВИЛЬНАЯ ИНИЦИАЛИЗАЦИЯ МОДЕЛИ ---
gemini_client = None
if not API_KEY:
    # Выводим ошибку, если ключ не найден
    logging.error("GOOGLE_API_KEY is missing from environment. Check your .env file.")
else:
    try:
        ...
        logging.info("Gemini AI client for chat configured successfully.")
...
def get_gemini_response(user_message: str) -> str:
    """
    Вызывает Gemini API с сообщением пользователя, используя предварительно настроенную модель.
    """
    if not gemini_client:
        return "AI сервис временно недоступен из-за ошибки конфигурации."

    try:
        # --- ГЛАВНОЕ ИЗМЕНЕНИЕ №3: ПРАВИЛЬНЫЙ ВЫЗОВ AI ---
        # Просто передаем сообщение пользователя, т.к. системные инструкции уже встроены в модель
        response = gemini_client.generate_content(user_message)
        return response.text
        
    except Exception as e:
        logging.error(f"Unexpected error in get_gemini_response: {e}")
        return "Внутренняя ошибка при обращении к ИИ."